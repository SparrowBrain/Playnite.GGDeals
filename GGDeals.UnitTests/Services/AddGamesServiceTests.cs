...
namespace GGDeals.UnitTests.Services
{
    public class AddGamesServiceTests
    {
            ...
            AddGamesService sut)
        {
            ...
            // Act
            var result = await sut.TryAddToCollection(games, reportProgress, ct);
            ...
            AddGamesService sut)
        {
            ...
            // Act
            await sut.TryAddToCollection(games, reportProgress, ct);
        ...
        [Theory]
        [AutoMoqData]
        public async Task TryAddToCollection_ReportsProgress_ForEveryRequestProcessed(
            [Frozen] Mock<IGameToAddFilter> gameToAddFilterMock,
            [Frozen] Mock<IGameToGameWithLauncherConverter> gameToGameWithLauncherConverterMock,
            [Frozen] Mock<IRequestDataBatcher> requestDataBatcherMock,
            [Frozen] Mock<IGGDealsApiClient> ggDealsApiClientMock,
            ...
            AddGamesService sut)
        {
            ...
            var reportProgress = new Action<float>(x => { progresses.Add(x); });
            ...
            Assert.Collection(progresses,
                x => Assert.Equal(0, x),
                x => Assert.Equal(100f / 3, x),
                x => Assert.Equal(200f / 3, x),
                x => Assert.Equal(100f, x));
        }
    }
}